...
public class Deploy_v2
{
    ...
    private static final int BRANCH_MAX_ITER = 15;
    
    // maximal iteration of sub-gradient search if backtracking happens
    private static final int BACKTRACK_MAX_ITER = 30;
    
    // time limit in milliseconds
    private static final long TIME_LIMIT = 89000L;
    ...
    public static String[] deployServer(String[] graphContent)
    {
        ...
        while (System.currentTimeMillis() - startTime <= TIME_LIMIT) {
            ...
            while (union.containsAll(directLinkNodes)) {
                tree.branch(chooseBranchNode(network, curNode), curNode.lowerBound, 0);
                curNode = tree.getLast();
            ...
            */
            if (totalIter == 1) {
                ...
                for (int k = 0; k < numOfClients; k++) {
                    directLink.add(mappingOfClients[k]);
                }
                for (int i = 0; i < numOfNodes; i++) {
                    if (!directLink.contains(i)) {
                        curNode.close(i);
                    }
                }
            }
                ...
                if (!curNode.getUndecided().isEmpty()) {
                    ...
                    if  (bandCost + openDeviceCost < upperBound) {
                        ...
                        while (true) {
                            ...
                            for (int i = 0; i < numOfNodes; i++) {
                                if (openDSEven.contains(i) || openDSOdd.contains(i)) {
                                    count[i]++;
                                }
                            }
                            ...
                            if (curNode.lowerBound <= upperBound - 1) {
                                ...
                                if (dualResult < curNode.lowerBound) {
                                    for (int i = 0; i < numOfNodes; i++) {
                                        accum[i] = 0.5 * accum[i] + network.getNode(numOfNodes, i).modDeviceCost;
                                    }
                                    stall++;
                                } else {
                                    stall = 0;
                                ...
                                iterLimit = penaltyTest(curNode, dualResult, iterLimit);
                        ...
                        // Alpha fixing
                        for (int i = 0; i < numOfNodes; i++) {
                            if (count[i] >= (1 - ALPHA) * iterLimit) {
                                curNode.open(i);
                            } else if (count[i] <= ALPHA * iterLimit) {
                                curNode.close(i);
                            }
                        }
                        */
                        ...
                        int largestIndex = 0;
                        double largest = 0;
                        while (m < Math.min(curNode.getUndecided().size(), BETA * numOfNodes)) {
                            for (int i = 0; i < numOfNodes; i++) {
                                if (curNode.getUndecided().contains(i) && Math.abs(accum[i]) > largest) {
                                    largest = Math.abs(accum[i]);
                                    largestIndex = i;
                                }
                            }
                            if (accum[largestIndex] < 0) {
                                curNode.open(largestIndex);
                            } else {
                                curNode.close(largestIndex);
                            }
                            m++;
                        }
                        */
                        if (needBranch) {
                            ...
                            //Object[] undecided = curNode.getUndecided().toArray();
                            //System.out.println((int) (Math.random() * undecided.length) - 2);
                            //int brachIndex = (int) undecided[(int) (Math.random() * (undecided.length - 1))];
    ...
    private static int penaltyTest(TreeNode tNode, double dualResult, int iterLimit) {
        ...
        return Math.min(ROOT_MAX_ITER, iterLimit + 1);
